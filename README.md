...
```
Install NCCL if needed
```bash
python -m cupyx.tools.install_library --library nccl --cuda 11.x
```
Install Alpa and Alpa-modified Jaxlib
```
pip3 install alpa
...
# Save files to disk
tokenizer.save("./norwegian-llama/tokenizer.json")
```
...
config.save_pretrained("./norwegian-llama")
```
...
python3 run_clm_flax.py \
    ...
    --tokenizer_name="./norwegian-llama" \
    --dataset_name="oscar" \
